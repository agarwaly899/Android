...
//                    dbReference.child(id).setValue(users);
                    ...
                    if (pass.length()>=8) {
                        ...
                        Toast.makeText(SignUp.this, "Password length must be greater than 8 !", Toast.LENGTH_LONG).show();
                        progressDialog.dismiss();
                    }
                }
            }
        ...
        firebaseAuth.signInWithEmailAndPassword(e, p).addOnCompleteListener(this, new OnCompleteListener<AuthResult>() {
            ...
            public void onComplete(@NonNull Task<AuthResult> task) {
                if (task.isSuccessful()) {
                    ...
                    UserInfo users = new UserInfo(fbuser.getUid(), u, pass, email, m1, m2);
                    dbReference.child(fbuser.getUid()).setValue(users);
                    finish();
                    startActivity(new Intent(SignUp.this, VehicleActivity.class));
                } else {
                    Toast.makeText(SignUp.this, "Something Went Wrong!", Toast.LENGTH_SHORT).show();
                    progressDialog.dismiss();
                }
